  ...
  return (
    <div className="min-h-screen flex flex-col p-4 md:p-6 animate-slide-up">
      <div className="max-w-lg mx-auto w-full flex-1 flex flex-col">
        ...
        <div
          className={cn(
            'game-card text-center py-6 mb-4 animate-bounce-in',
            isCorrect
        ...
        >
          <div className="flex justify-center mb-3">
            {isCorrect ? (
              ...
              <XCircle className="w-16 h-16 text-trap" />
            )}
          </div>
          <h2 className={cn('text-2xl font-bold', isCorrect ? 'text-trust' : 'text-trap')}>
        ...
        {/* Explanation */}
        <div className="game-card mb-4 bg-primary/5 border-primary/20">
          <h3 className="text-sm font-semibold text-muted-foreground mb-2 uppercase tracking-wide flex items-center gap-2">
            <Lightbulb className="w-4 h-4" /> Why this is a {message.correctVerdict}
          </h3>
          <p className="text-foreground leading-relaxed">{message.explanation}</p>
        </div>

        {/* Advice */}
        <div className="game-card mb-6 bg-warning/10 border-warning/30">
          <h3 className="text-sm font-semibold text-muted-foreground mb-2 uppercase tracking-wide flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-warning" /> What to do instead
          </h3>
          <p className="text-foreground leading-relaxed">{message.advice}</p>
        </div>
